...
# Constant value to all bacterial prots fasta file
PROTS_PATH = Path.cwd().joinpath('analysis_files/bacterial_protein_sequences.fasta')
...
class Protein:
    ...
    def extractSpecies(self, entry):
        ...
        taxaList = entry[stringStart:stringEnd].split(';')
        taxaDB = set()
        for t in taxaList:
            taxaDB.add(t)
        return taxaDB
    ...
    def getEntry(self):
        taxaEntry = self.getTaxaString()
        return f'>{self.id} {self.name} OS={taxaEntry} {self.data}\n{self.sequence}\n'
    
    # Returns a string of all the taxa associated with this protein, separated by ';' characters
...
class Spectrum:
    def __init__(self, resRow):
        ...
        self.hits = getProteinHitList(resRow)
        ...
        with resultFile.open(mode='r') as tsvin:
            ...
            for row in tsvReader:
                ...
                if protType != 'contaminant' and protType != 'decoy':
                    ...
                    for hit in getProteinHitList(row):
            ...
            with tsvFile.open(mode='r') as infile:
                ...
                for row in tsvReader:
                    ...
                    if float(row[Q_VALUE]) > 0.01:
                        break
                    if determineHitType(row) != 'bacteria':
                        continue
...
# Takes a spectral hit and determines the type of protein it is.
...
def determineHitType(spectrumRow):
    protType = None
    for protID in getProteinHitList(spectrumRow):
        if protID == 'Protein':
            return 'first'
        if protID.find('XXX') != -1:
            return 'decoy'
        if protID.find('Contaminant_') != -1:
            protType = 'contaminant'
        elif protID.find('HUMAN') != -1 and (protType == None or protType == 'bacteria' or protType == 'fungi' or protType == 'trichomonas'):
            protType = 'human'
        elif protID.find('FUNG_') != -1 and (protType == None or protType == 'trichomonas'):
            protType = 'fungi'
        elif protID.find('TRICH_') != -1 and protType == None:
...
def isStringent(spectrumRow, protRef, genus=False):
    ...
    if len(spectrumRow[PEPTIDE].replace('+15.995', '')) < 10:
        return False
    hits = getProteinHitList(spectrumRow)
    if len(hits) == 1:
        return True
...
# Returns the mean of the numbers in listVals
def mean(listVals):
...
# Deprecated
def dgetFilteredPeptides(results, typeOfProt):
    proteins = {} # key=protein id, value=set of peptide sequences
    pepCounts = {} # key=peptide sequence, value=number of times identified across all samples
    pepProbabilities = {} # key=peptide sequence, value=lowest spectral probability
    if not isinstance(results, list):
...
def generateNormalizedStackedBar(samples, categories, data, title='', ylabel='Percent Detected Spectra', xLabTotals=False, sort=False):
    toGraph = data.copy()
...
def setPlt(size=(0, 0), xlabel='', ylabel='', title='', xticksrot=0):
    if size != (0, 0):
